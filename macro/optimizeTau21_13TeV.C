...
TGraph* decorateGraph(TGraph *graph, TString xTitle, TString yTitle, TString myTitle, double yMin, double yMax, int color){
  ...
  graph->GetXaxis()->SetRangeUser(0.44, 0.8);
...
void optimizeTau21_13TeV(){
  vector<string>massPoints;
  massPoints.push_back("all_ExLepMuMuZ_M250.root");
  massPoints.push_back("all_ExLepMuMuZ_M500.root");
  massPoints.push_back("all_ExLepMuMuZ_M750.root");
  massPoints.push_back("all_ExLepMuMuZ_M1000.root");
  massPoints.push_back("all_ExLepMuMuZ_M1250.root");
  massPoints.push_back("all_ExLepMuMuZ_M1500.root");
  massPoints.push_back("all_ExLepMuMuZ_M1750.root");
  massPoints.push_back("all_ExLepMuMuZ_M2000.root");
  massPoints.push_back("all_ExLepMuMuZ_M2500.root");
  massPoints.push_back("all_ExLepMuMuZ_M3000.root");
  massPoints.push_back("all_ExLepMuMuZ_M3500.root");
  massPoints.push_back("all_ExLepMuMuZ_M4000.root");
  massPoints.push_back("all_ExLepMuMuZ_M4500.root");
  massPoints.push_back("all_ExLepMuMuZ_M5000.root");
  ...
  TLegend* leg = new TLegend(0.60,0.10,0.90,0.50,NULL,"brNDC");
  leg->SetBorderSize(0);
  ...
  for(int i = 0; i<massPoints.size(); i++){
   ...
   int color = i+1;
   if(i==9) color = 433;
   decorateGraph(myGraph, "Tau21", "100* effSig/sqrt(nBkg)", "optimization of tau21: muon channel", 2.8, 5.1, color);
   //decorateGraph(myGraph, "Tau21", "effSig/effBkg", "optimization of tau21: muon channel", 0.7, 1.2, color);
   if(i==0) myGraph->Draw("ALP");
   else myGraph->Draw("LPsame");
   leg->AddEntry(myGraph, TString(massLables[i]),"PL");
  }
  leg->Draw();
  can->SaveAs("tau21.png");
}
